...
% GOTO:
%   - 'HOME', [],   [],   '',   T_wait  -> về q_home, dừng T_wait
%   - [x y z], v, a, mode, T_wait -> đi tới XYZ, dừng T_wait ở điểm đó
...
% ====== CHUYỂN GOTO -> WP (hỗ trợ 'HOME' + T_wait) ======
n_cmd   = size(GOTO, 1);
WP_cell = {};
for i = 1:n_cmd
    dest   = GOTO{i,1};
    ...
    if ischar(dest) || isstring(dest)
        ...
        wp.T_wait = T_wait;      % thời gian dừng tại HOME
        WP_cell{end+1} = wp;
    else
        % Dạng [x y z]
        WP_cell{end+1} = make_wp(dest(1), dest(2), dest(3), v, a, mode, T_wait); 
    end
end
...
% =====================================================================
% TẠO 1 WAYPOINT: pos + vận tốc + gia tốc + mode + T_wait
% =====================================================================
function wp = make_wp(px, py, pz, v, a, mode, T_wait)
    if nargin < 7, T_wait = []; end
...
% =====================================================================
function [P, v, a, mode, T_wait] = unpack_wp(wp, cfg)
    % mode
    ...
    % thời gian dừng
    if isfield(wp, 'T_wait') && ~isempty(wp.T_wait)
        T_wait = wp.T_wait;
    else
        T_wait = cfg.T_wait;   % fallback về mặc định
    end
end
...
function [q_traj, qd_traj, qdd_traj] = plan_path_with_home_wp(WP, q_home, cfg, geom, dt, my_Robot)
    ...
    for i = 1:(n_wp - 1)
        ...
        % Dừng tại điểm đích theo T_wait của wp_end
        N_WAIT = round(T_wait_end / dt);
...
% CHECK GIỚI HẠN KHỚP
% =====================================================================
function q_out = check_joint_limits(q_in)
    % Giới hạn giống create_robot
    LIMITS_DEG = [-170, 170; -170, 170; -170, 170; -170, 170; -170, 170; -170, 170];
    LIMITS_RAD = deg2rad(LIMITS_DEG);
    q_out = q_in;
    if any(q_in < LIMITS_RAD(:,1).' | q_in > LIMITS_RAD(:,2).')
        fprintf('[IK] Vuot gioi han: q_deg = [%6.1f %6.1f %6.1f %6.1f %6.1f %6.1f]\n', rad2deg(q_in));
        q_out = nan(1,6);
    end
end
